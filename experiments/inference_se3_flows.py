...
from models.adapter_flow_module import FlowModule
...
class EvalRunner:
    ...
    def run_sampling(self):
        devices = GPUtil.getAvailable(
            order='memory', limit = 4, maxLoad=0.5, maxMemory=0.5)[:self._infer_cfg.num_gpus]
        ...
        elif self._infer_cfg.task =='EnzymeInpainting':
            eval_dataset = eu.EnzymeInpaintingDataset(self._samples_cfg)
        elif self._infer_cfg.task =='LiLei':
            eval_dataset = eu.LiLeiDataset(self._samples_cfg)
        else:
...
@hydra.main(version_base=None, config_path="../configs", config_name="inference_enzyme")
