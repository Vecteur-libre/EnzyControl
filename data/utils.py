...
def parse_chain_feats(chain_feats, scale_factor=1.):
    '''
    1. 提取alpha 碳原子的索引
    2. 修改bb_mask，来存储backbone的掩码信息
    3. 从原子坐标中提取alpha 碳原子的pos.  shape = (num_residues, 3)
    4. 基于alpha碳原子的pos得到中心化坐标bb_center
    5. 将每个原子的坐标减去bb_center,完成中心化操作，使蛋白质的集合中心对齐到原点。
    6. 对坐标做一下放缩
    7. mask不存在的原子，再扩展一下维度
    '''
    ca_idx = residue_constants.atom_order['CA']
    chain_feats['bb_mask'] = chain_feats['atom_mask'][:, ca_idx]
    bb_pos = chain_feats['atom_positions'][:, ca_idx]#提取alpha碳原子的pos
